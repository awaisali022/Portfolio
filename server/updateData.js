...
fs.writeFileSync(path.join(dataDir, 'experiences.json'), JSON.stringify(experiences, null, 2));

// Update educations.json
const educations = [
  {
    id: generateUUID(),
...
fs.writeFileSync(path.join(dataDir, 'educations.json'), JSON.stringify(educations, null, 2));
...
const mappedSkills = skills.map((s, i) => ({
  id: generateUUID(),
  ...s,
  created_at: now,
  updated_at: now
}));
fs.writeFileSync(path.join(dataDir, 'skills.json'), JSON.stringify(mappedSkills, null, 2));
