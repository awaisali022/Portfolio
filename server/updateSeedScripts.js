...
const seedLocalPath = path.join(__dirname, 'scripts', 'seed-local.js');
...
const projectsBlockLocal = `  const projects = [
    {
...
const experiencesBlockLocal = `  const experiences = [
    {
...
const educationsBlockLocal = `  const educations = [
    {
...
const skillsBlockLocal = `  const skills = [
...
function updateSeedLocal() {
  let content = fs.readFileSync(seedLocalPath, 'utf8');

  // Regex replacers for local
  content = content.replace(/heroTitle: 'Awais Ali',[\s\S]*?emailAddress: '.*?',/, settingsBlock);
  content = content.replace(/const projects = \[[\s\S]*?\];/m, projectsBlockLocal);
  content = content.replace(/const experiences = \[[\s\S]*?\];/m, experiencesBlockLocal);
  content = content.replace(/const educations = \[[\s\S]*?\];/m, educationsBlockLocal);
  content = content.replace(/const skills = \[[\s\S]*?\];/m, skillsBlockLocal);

  fs.writeFileSync(seedLocalPath, content, 'utf8');
}

updateSeedLocal();
console.log('Done replacing seed-local.js');
